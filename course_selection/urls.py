...
urlpatterns = patterns(
    ...
    url(
        r"^course_evaluations/(?P<semester_id>\d+)/(?P<course_id>\d+)$",
        views.course_evaluations,
        name="course_evaluations",
    ),
    url(r"^api/", include(v1_api.urls)),
    url(
        r"^api/static/courses/(?P<term_code>\d+)$",
        views.get_courses_json,
        name="get-courses-json",
    ),
    url(
        r"^api/static/courses", views.get_courses_json_old, name="get-courses-json-old"
    ),
    ...
    url(
        r"^api/worksheet/(?P<schedule_id>\d+)$",
        views.get_worksheet_pdf,
        name="get-worksheet-pdf",
    ),
    url(r"^ical/(?P<cal_id>.*)\.ics$", views.ical_feed, name="ical-feed"),
    url(
        r"^icalapi/geturl/(?P<schedule_id>\d+)$",
        views.get_ical_url_for_schedule,
        name="ical-get-url",
    ),
    url(
        r"^icalapi/regenerate/(?P<schedule_id>\d+)$",
        views.regenerate_ical_url_for_schedule,
        name="ical-regenerate-url",
    ),
)
