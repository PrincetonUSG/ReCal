...
def index(request):
    ...
    return render(
        request, "main/index.html", {"username": unicode(request.user.username)}
    )
...
def continuity_check(request):
    ...
    if (
        Schedule.objects.count() > 100
        and Section.objects.count() > 100
        and Nice_User.objects.count() > 100
    ):
...
def hydrate_course_dict(course):
    sections = [
        hydrate_section_dict(section, course) for section in course.sections.all()
    ]
    course_listings = [
        hydrate_course_listing_dict(cl) for cl in course.course_listing_set.all()
    ]
    return {
...
def hydrate_user_dict(user):
    return {"id": user.id, "netid": user.netid}
...
@cache_page_with_prefix(
    60 * 60 * 24,
    lambda request: hashlib.md5(request.GET.get("semester__term_code", "")).hexdigest(),
)
...
def get_worksheet_pdf(
    request, schedule_id, template_name="course_enrollment_worksheet.pdf", **kwargs
):
    """
    ...
    response["Content-Disposition"] = "inline; filename=course_enrollment_worksheet.pdf"
...
def fill_out_course(context, idx, enrollment):
    ...
    sections = [
        Section.objects.get(id=section_id) for section_id in enrollment["sections"]
    ]
    ...
    for section in sections:
        ...
        # TODO: test this
        if (
            section.section_type == Section.TYPE_LECTURE
            or section.section_type == Section.TYPE_SEMINAR
            or section.section_type == Section.TYPE_CLASS
        ):
            section_type = "a"
    ...
    for course_obj in json.loads(sched.enrollments):
        ...
        for section_id in course_obj["sections"]:
            ...
            for meeting in section.meetings.all():
                ...
                assert all([d >= 0 for d in dayofweek_relative_to_semester_start])
                # a T,Th class will have first meeting on T if semester starts
                # on M, or on Th if semester starts on Wed.
                first_meeting_dayofweek = min(dayofweek_relative_to_semester_start)
                ...
                selected_days = [
                    ical_days[i] for i in sorted(daysofweek)
                ]  # formatted for ical
...
def get_ical_url(request, schedule_id, make_new=False):
    ...
    return HttpResponse(
        request.build_absolute_uri(
            reverse("ical-feed", args=(str(schedule.ical_uuid),))
        )
    )
