class ScrapeCounter:
    ...
    def __str__(self):
        return (
            ...
            + " new courses\n"
            + str(self.totalCoursesCount)
            + " total courses\n"
            + str(self.createdSectionsCount)
            + " new sections\n"
            + str(self.totalSectionsCount)
            + " total sections\n"
            + str(self.createdMeetingsCount)
            + " new meetings\n"
            + str(self.totalMeetingsCount)
            + " total meetings\n"
            + str(self.createdProfessorsCount)
            + " new professors\n"
            + str(self.totalProfessorsCount)
            + " total professors\n"
            + str(self.createdListingsCount)
            + " new listings\n"
            + str(self.totalListingsCount)
            + " total listings"
        )
...
def scrape_import_course(course, counter=ScrapeCounter()):
    ...
    def import_section(section, course_object):
        ...
        section_object, created = Section.objects.get_or_create(
            course=course_object, name=section["name"]
        )
    ...
    def import_professor(prof, course_object):
        ...
        prof_object, created = Professor.objects.get_or_create(name=prof["full_name"])
    ...
    course_object, created = Course.objects.get_or_create(
        registrar_id=course["guid"], semester=import_semester(course["semester"])
    )
